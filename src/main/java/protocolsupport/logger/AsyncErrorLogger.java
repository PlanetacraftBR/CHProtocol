...
public class AsyncErrorLogger {
	...
	private static final String filePath = Utils.getJavaPropertyValue("protocolsupport.errlog.path", JavaPlugin.getPlugin(Main.class).getName()+"-errlog", Converter.NONE);
	...
	private final ExecutorService executor = Executors.newSingleThreadExecutor(new ThreadFactory() {
		...
		public Thread newThread(Runnable r) {
			return new Thread(r, JavaPlugin.getPlugin(Main.class) + "-errlog-thread");
		}
	});
	...
	private AsyncErrorLogger() {
		...
		try {
			...
			Main.logWarning("Unable to create error log");
			stop();
		}
	}
